...
def tumor_models(tumor, device) -> list:
    ...
    countModel = torch.load(f"/genomeGAN/simulation/trained_models/counts/{tumor}_counts.pkl", map_location=device)
    mutModel = torch.load(f"/genomeGAN/simulation/trained_models/mutations/{tumor}_mutations.pkl", map_location=device)

    if tumor == "Lymph-CLL":
        posModel:dict = {}
        with open(f"/genomeGAN/simulation/trained_models/positions/{tumor}_MUT_positions.pkl", 'rb') as f:
           posModel['MUT'] = pickle.load(f)
        with open(f"/genomeGAN/simulation/trained_models/positions/{tumor}_UNMUT_positions.pkl", 'rb') as f:
           posModel['UNMUT'] = pickle.load(f)
    else:
        with open(f"/genomeGAN/simulation/trained_models/positions/{tumor}_positions.pkl", 'rb') as f:
...
def simulate_vaf_rank(tumor, nCases) -> list:
    ...
    rank_file:pd.DataFrame = pd.read_csv("/genomeGAN/simulation/trained_models/vaf_rank_list.tsv", sep='\t') 
...
def get_sexual_chrom_usage(tumor, gender) -> dict:
    ...
    sexChrFile:pd.DataFrame = pd.read_csv("/genomeGAN/simulation/trained_models/xy_ranks.txt", sep='\t') 
...
def simulate_mut_vafs(tumor, rank, n) -> list: 
    ...
    prop_vaf_file:pd.DataFrame = pd.read_csv(f"/genomeGAN/simulation/trained_models/vaf_annotation_by_study.tsv", sep='\t')
