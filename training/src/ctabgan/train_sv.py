...
def trainSV(csv, prefix, outdir, epochs, batch_size, test_ratio, lr):
    ...
    synthesizer = CTABGAN(raw_csv_path = csv,
                          ...
                          categorical_columns = ['strand1', 'strand2', 'svclass'],
                          log_columns = ['len'],
                          mixed_columns = {},
                          general_columns= [],
                          integer_columns = ['start'],
                          problem_type= {"Classification": "svclass"},
                          epochs = epochs,
    ...
    if len(syn) < n:
        ...
        save(synthesizer, f"{outdir}/{prefix}_SV_epoch{epochs}_batchsize{batch_size}_lr{lr}_testratio{test_ratio}.pkl")
        syn.to_csv(f"{outdir}/{prefix}_SV_epoch{epochs}_batchsize{batch_size}_lr{lr}_testratio{test_ratio}.txt", sep="\t", index=False)
